...
let data = {
    ...
    reserveTime: "2024-04-09 14:00:02", // 建议设置偏移2S
    userTimeOffset: ".200",
        ...
        let timer = setInterval(function () {
            ...
            if (addPerson) {
                ...
                setTimeout(function () {
                    ...
                    console.log('获取到的输入框', inputs);
                    while (inputs.length != data.uList.length * 2) {
                    ...
                    inputs.forEach((input, index) => {
                        if (index % 2 === 0) {
                            input.value = data.uList[uIndex].uname;
                            input.dispatchEvent(new Event('input', {
                                bubbles: true
                            }));
                        } else {
                            input.value = data.uList[uIndex].uid;
                            input.dispatchEvent(new Event('input', {
                                bubbles: true
                            }));
                            uIndex++;
                    ...
                    // 等待提交结果
                    let timer = setInterval(function () {
                        panelObj.setMsg('第 ' + ++count + ' 次提交');
                        const msgBox = document.querySelector('.confirm-layout .tips');
                        const poptip = document.querySelector('.poptip .content');
                        if ((msgBox && msgBox.innerHTML === '预约成功') || (poptip && poptip.innerHTML === '观众人数已达上限')) {
                            panelObj.setMsg((msgBox && msgBox.innerHTML) || (poptip && poptip.innerHTML));
                            clearInterval(timer);
                            return;
                        }
                        submBtn.click();
